    ...
    <div class="page-header">
      <div class="add-item d-flex">
        <div class="page-title">
          ...
          <h6>Manage customer outstanding payment and due aging</h6>
        </div>
      </div>
        ...
        <div class="table-responsive">
          <table class="table datanew">
            <thead>
              <tr>
                <th>Customer Name</th>
                <th class="text-center">Order Due</th>
            ...
            <tbody>
              <tr v-for="item in filteredDues" :key="item.name">
                ...
                <td class="text-center">{{ item.orderDue }}</td>
                <td class="text-end fw-bold text-danger">{{ formatNumber(item.amountDue) }}</td>
                <td class="text-center">
                  <span
                    class="badge"
                    :class="parseInt(item.daysDue) > 10 ? 'bg-outline-danger' : 'bg-outline-warning'"
                  >
                    {{ item.daysDue }}
                  </span>
                </td>
            ...
            <tfoot>
              <tr class="fw-bold table-light">
                <td>Total</td>
                <td class="text-center">{{ totalOrdersDue }}</td>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header border-0 pb-0">
            <h4 class="modal-title">Outstanding Invoices - {{ activeItem?.name }}</h4>
            <button type="button" class="btn-close" @click="showModal = false"></button>
          </div>
          <div class="modal-body p-4" v-if="activeItem">
            <p><strong>Customer:</strong> {{ activeItem.name }}</p>
            <p><strong>Total Pending Orders:</strong> {{ activeItem.orderDue }}</p>
            <p><strong>Total Due Amount:</strong> Rp {{ formatNumber(activeItem.amountDue) }}</p>
            <p><strong>Overdue Aging:</strong> {{ activeItem.daysDue }}</p>
...
const totalOrdersDue = computed(() => filteredDues.value.reduce((acc, d) => acc + d.orderDue, 0))
